...
/**
 * File containing the ContentTypeGroupIdIn criterion visitor class.
 *
...
class ContentTypeGroupIdIn extends CriterionVisitor
{
    ...
    {
        return
            $criterion instanceof Criterion\Matcher\ContentTypeGroupId &&
            (
    ...
    public function visitFilter(CriterionInterface $criterion, Dispatcher $dispatcher, array $languageFilter)
    {
        ...
        if (count($criterion->value) > 1) {
            $filter = array(
                'terms' => array(
                    'content_group_mid' => $criterion->value,
                ),
            );
        } else {
            $filter = array(
                'term' => array(
                    'content_group_mid' => $criterion->value[0],
                ),
            );
        }
