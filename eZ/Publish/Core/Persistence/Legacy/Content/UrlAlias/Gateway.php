...
abstract class Gateway
{
     ...
     */
    abstract public function updateRow($parentId, $textMD5, int $languageMask, array $values);
     ...
     * @param array $values
     *
     * @throws \eZ\Publish\API\Repository\Exceptions\BadStateException if an alias to be inserted is not unique and numeric suffix needs to be added
     *
     * @return mixed
     */
    ...
    abstract public function loadCurrentAliasRow(int $parentId, string $textHash): array;
     ...
     * @return array
     */
    abstract public function loadUrlAliasData(array $urlHashes, ?int $languageMask = null);

    /**
     * Loads autogenerated entry id by given $action and optionally $parentId.
     *
     * @param int|null $languageMask if given, filter by a matching language mask
     * (excluding matches on always available only)
     */
    abstract public function loadAutogeneratedEntry(
        string $action,
        ?int $parentId = null,
        ?int $languageMask = null
    ): array;
    ...
    /**
     * Load rows for a path (a path element text constrained by a parent).
     *
     * URL Alias entries for a given path can differ by language mask only.
     */
    abstract public function loadSystemEntriesForPath(int $parentId, string $textHash): array;
}
