...
class Handler implements UrlAliasHandlerInterface
{
    ...
    public function locationSwapped($location1Id, $location1ParentId, $location2Id, $location2ParentId)
    {
        ...
        $alwaysAvailable1 = $this->maskGenerator->isAlwaysAvailable($contentInfo1['language_mask']);
        $alwaysAvailable2 = $this->maskGenerator->isAlwaysAvailable($contentInfo2['language_mask']);
        ...
        foreach ($languages as $languageCode => $language) {
            $isMainLanguage1 = $language->id == $location1MainLanguageId;
            $isMainLanguage2 = $language->id == $location2MainLanguageId;
            if (isset($names1[$languageCode], $names2[$languageCode])) {
                $name1 = $names1[$languageCode];
                $name2 = $names2[$languageCode];
                if ($this->shouldUrlAliasForSecondLocationBePublishedFirst(
                    $location1Entries,
                    $location1ParentId,
                    $name1,
                    $location2Entries,
                    ...
                    $this->internalPublishUrlAliasForLocation(
                        ...
                        $name2,
                        $language->id,
                        $isMainLanguage1 && $alwaysAvailable2,
                        $isMainLanguage1,
                        $autoLocation2['id']
                    );
                    ...
                    $this->internalPublishUrlAliasForLocation(
                        ...
                        $name1,
                        $language->id,
                        $isMainLanguage2 && $alwaysAvailable1,
                        $isMainLanguage2,
                        $autoLocation1['id']
                    );
                } else {
            ...
            } elseif (isset($names1[$languageCode])) {
                $this->internalPublishUrlAliasForLocation(
                    $location1Id,
                    $location1ParentId,
                    $names1[$languageCode],
            ...
            } elseif (isset($names2[$languageCode])) {
                $this->internalPublishUrlAliasForLocation(
                    $location2Id,
                    $location2ParentId,
                    $names2[$languageCode],
