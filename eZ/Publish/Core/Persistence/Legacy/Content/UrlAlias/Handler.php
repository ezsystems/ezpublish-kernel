...
class Handler implements UrlAliasHandlerInterface
{
     ...
     */
    protected function copySubtree($actionMap, $oldParentAliasId, $newParentAliasId, array &$alreadyGeneratedPairs = [])
    {
        $rows = $this->gateway->loadAutogeneratedEntries($oldParentAliasId);
        $newIdsMap = [];
        foreach ($rows as $row) {
            $identifier = $row['parent'] . $row['text_md5'];
            if (isset($alreadyGeneratedPairs[$identifier])) {
                continue;
            }
            ...
            $alreadyGeneratedPairs[$identifier] = true;
