...
class Handler implements UrlAliasHandlerInterface
{
     ...
     * @param int|null $forcedEntryId {@see \eZ\Publish\Core\Persistence\Legacy\Content\UrlAlias\Handler::locationSwapped}
     */
    private function internalPublishUrlAliasForLocation(
        ...
        bool $updatePathIdentificationString = false,
        ?int $forcedEntryId = null
    ): void {
        ...
        $action = $this->getSystemUrlAliasAction($locationId);
        $cleanup = false;

        $newId = $newText = $newTextMD5 = null;
        do {
            ...
            if (!empty($existingLocationEntry)) {
                $cleanup = true;
                $forcedEntryId = $existingLocationEntry['id'];
            }

            try {
                $newId = $this->generateSystemUrlAlias(
                    $parentId,
                    $newTextMD5,
                    $newText,
                    $languageMask,
    ...
    protected function createUrlAlias(
        ...
        // Handle all path elements except the topmost one
        foreach ($pathElements as $level => $pathElement) {
            $parentId = $this->ensurePathHasElement($parentId, $level, $pathElement);
        }
        $topElement = $this->slugConverter->convert($topElement, 'noname' . (count($pathElements) + 1));
        $topElementHash = $this->getHash($topElement);
        $topElementRow = $this->gateway->loadRow($parentId, $topElementHash, $languageId);
        // if row exists, check if it is reusable. There are 2 cases when this is possible:
        // 1. NOP entry
        // 2. history entry
        if (
            !empty($topElementRow)
            && $topElementRow['action_type'] !== 'nop'
            && (int)$topElementRow['is_original'] === 1
        ) {
        ...
        $data = $this->createCustomUrlAliasElement(
            $parentId,
            $action,
            $topElement,
            $topElementHash,
    ...
    /**
     * Generate system URL Alias by either creating or reusing existing system (is_alias=0) entry.
     *
     * @return int new ID
     *
     * @throws \eZ\Publish\API\Repository\Exceptions\BadStateException
     */
    private function generateSystemUrlAlias(
        int $parentId,
        string $newTextMD5,
        string $newText,
        int $languageMask,
        ...
        if (null !== $forcedEntryId) {
            $data['id'] = $forcedEntryId;
        }
        $existingEntry = $this->loadExistingPathEntryData(
            $parentId,
            $newTextMD5,
            $languageMask,
            $action
        );
        // override any previous data ("nop" entries become "eznode", aliases become system ones)
        if (!empty($existingEntry)) {
            $data['link'] = $forcedEntryId ?? (int)$existingEntry['id'];
            // "& ~1" to avoid copying always available if the new mask doesn't have it
            $data['lang_mask'] |= ((int)$existingEntry['lang_mask']) & ~1;

            if ($this->entryIsOutdated($existingEntry, $data)) {
                $this->gateway->updateRow(
                    $parentId,
                    $newTextMD5,
                    $existingEntry['lang_mask'],
                    $data
                );
            }

            // reuse found system URL
            return $data['link'];
        }
        ...
        return $this->gateway->insertRow(
            array_merge(
                [
                    'id' => $forcedEntryId, // generated when null
                    'parent' => $parentId,
                    'text_md5' => $newTextMD5,
                ],
                $data
    ...
    private function getSystemUrlAliasAction(int $locationId): string
    {
        return 'eznode:' . $locationId;
    }

    private function entryIsOutdated(array $entry, array $currentData): bool
    {
        foreach ($currentData as $columnName => $value) {
            if ($entry[$columnName] !== $value) {
                return true;
            }
        }
    ...
    /**
     * Check if the given path element exists and create "nop:" entry if not.
     *
     * Note: topmost element should not be passed to this method.
     */
    private function ensurePathHasElement(int $parentId, int $level, string $pathElement): int
    {
     ...
     * @throws \eZ\Publish\API\Repository\Exceptions\BadStateException
     */
    private function createCustomUrlAliasElement(
        int $parentId,
        string $action,
        string $topElement,
        string $topElementHash,
        array $topElementRow,
        int $languageId,
        bool $alwaysAvailable,
        bool $forward
        ...
        $data = [
            ...
            'parent' => $parentId,
            'text' => $topElement,
            'text_md5' => $topElementHash,
            'is_original' => 1,
        ];

        $currentLanguageMask = $languageId | (int)$alwaysAvailable;
        if (empty($topElementRow)) {
            // check if other rows are reusable (archived entries should remain untouched)
            $topElementRow = $this->gateway->loadCurrentAliasRow($parentId, $topElementHash);
        }

        if (empty($topElementRow)) {
            $data['lang_mask'] = $currentLanguageMask;
            $data['link'] = $this->gateway->insertRow($data);
        } else {
            $data['link'] = (int)$topElementRow['id'];
            $topElementLanguageMask = (int)$topElementRow['lang_mask'];
            $data['lang_mask'] = $currentLanguageMask | $topElementLanguageMask;
            $this->gateway->updateRow(
                $parentId,
                $topElementHash,
                $topElementLanguageMask,
                $data
            );
        }

        $data['raw_path_data'] = $this->gateway->loadPathData($data['link']);
     ...
     * @throws \eZ\Publish\API\Repository\Exceptions\BadStateException
     */
    private function loadExistingPathEntryData(
        int $parentId,
        string $textHash,
        int $languageMask,
        string $action
    ): array {
        $existingEntry = $this->gateway->loadRow($parentId, $textHash, $languageMask);
        $reusableActions = [$action, 'nop:'];
        if (!empty($existingEntry) && in_array($existingEntry['action'], $reusableActions, true)) {
            return $existingEntry;
        }

        // @todo move to db query
        // if there's no exact match, see if any other aliases qualify
        $urlAliasCandidates = $this->gateway->loadSystemEntriesForPath($parentId, $textHash);
        $matchingAliases = \array_filter(
            $urlAliasCandidates,
            static function (array $row) use ($languageMask): bool {
                $isSystemUrl = (int)$row['is_alias'] === 0;
                $isArchived = (int)$row['is_original'] === 0;
                $commonLanguageMask = (int)$row['lang_mask'] & $languageMask;

                // match system URLs which are not archived or archived for a matching mask
                return $isSystemUrl && (!$isArchived || $commonLanguageMask > 1);
            }
        );

        if (empty($matchingAliases)) {
            return [];
        }

        $matchingAlias = $matchingAliases[0];
        if (
            (int)$matchingAlias['is_original'] === 1 &&
            !in_array($matchingAlias['action'], $reusableActions, true)
        ) {
            throw new BadStateException(
                'path',
                sprintf(
                    'The URL Alias "%d-%s" is not reusable for the action "%s" because it supports the action "%s"',
                    $parentId,
                    $textHash,
                    $action,
                    $matchingAlias['action']
                )
            );
        }
