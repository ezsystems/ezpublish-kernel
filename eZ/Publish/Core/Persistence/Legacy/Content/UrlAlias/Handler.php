...
class Handler implements UrlAliasHandlerInterface
{
     ...
     */
    protected function copySubtree($actionMap, $oldParentAliasId, $newParentAliasId, array &$alreadyGeneratedAliases = [])
    {
        $rows = $this->gateway->loadAutogeneratedEntries($oldParentAliasId);
        $newIdsMap = [];
        foreach ($rows as $row) {
            $identifier = $row['parent'] . $row['text_md5'];
            if (isset($alreadyGeneratedAliases[$identifier])) {
                continue;
            }
            ...
            $alreadyGeneratedAliases[$identifier] = true;
