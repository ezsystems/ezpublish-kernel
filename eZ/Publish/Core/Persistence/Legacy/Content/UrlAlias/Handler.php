    ...
    public function locationSwapped($location1Id, $location1ParentId, $location2Id, $location2ParentId)
    {
        ...
        foreach ($location2Entries as $row) {
            ...
            foreach ($languageIds as $languageId) {
                ...
                $location1NewAliasId = $this->publishUrlAliasForLocation(
                    $location1Id,
                    $location1ParentId,
                    $row['text'],
                    $this->languageHandler->load($languageId)->languageCode,
                    $alwaysAvailable,
                    $alwaysAvailable
                );
            }
        ...
        foreach ($location1Entries as $row) {
            ...
            foreach ($languageIds as $languageId) {
                ...
                $location2NewAliasId = $this->publishUrlAliasForLocation(
                    $location2Id,
                    $location2ParentId,
                    $row['text'],
                    $this->languageHandler->load($languageId)->languageCode,
