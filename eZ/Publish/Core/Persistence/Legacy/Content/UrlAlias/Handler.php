...
class Handler implements UrlAliasHandlerInterface
{
     ...
     * @param string[] $alreadyGeneratedPairs
     */
    protected function copySubtree($actionMap, $oldParentAliasId, $newParentAliasId, &$alreadyGeneratedPairs = [])
    {
        $rows = $this->gateway->loadAutogeneratedEntries($oldParentAliasId);
        $newIdsMap = [];
        foreach ($rows as $row) {
            $temporaryRowIdentifier = $row['parent'] . $row['text_md5'];
            if (in_array($temporaryRowIdentifier, $alreadyGeneratedPairs, true)) {
                continue;
            }
            ...
            $alreadyGeneratedPairs[] = $temporaryRowIdentifier;

            $this->copySubtree(
                $actionMap,
                $oldParentAliasId,
                $row['id'],
                $alreadyGeneratedPairs
            );
        }
    }
