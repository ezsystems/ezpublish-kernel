...
class ExceptionConversion extends Gateway
{
    ...
    public function updateRow($parentId, $textMD5, int $languageMask, array $values)
    {
        try {
            $this->innerGateway->updateRow($parentId, $textMD5, $languageMask, $values);
        } catch (DBALException | PDOException $e) {
            throw DatabaseException::wrap($e);
        }
    }
    ...
    public function insertRow(array $values)
    {
        try {
            ...
            throw new BadStateException(
                'path',
                \sprintf(
                    'The URL Alias "%s-%s" for the given language mask %s already exists',
                    $values['parent'] ?? '?',
                    $values['text_md5'] ?? '?',
                    $values['lang_mask'] ?? '?'
                )
    ...
    {
        try {
            return $this->innerGateway->loadRow($parentId, $textMD5, $languageMask);
        } catch (DBALException | PDOException $e) {
            throw DatabaseException::wrap($e);
        }
    }
    ...
    {
        try {
            return $this->innerGateway->loadCurrentAliasRow($parentId, $textHash);
        } catch (DBALException | PDOException $e) {
            throw DatabaseException::wrap($e);
        }
    }
    ...
    {
        try {
            return $this->innerGateway->loadSystemEntriesForPath($parentId, $textHash);
        } catch (DBALException | PDOException $e) {
            throw DatabaseException::wrap($e);
        }
    }
