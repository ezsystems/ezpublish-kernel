...
class DoctrineDatabase extends Gateway
{
    ...
    public function updateContentTypeCopyIdentifier($typeId, $baseIdentifier)
    {
        ...
        $selectQueryWrapper = $updateQuery->subSelect();

        // create query to retrieve number of content type identifiers starting with $baseIdentifier
        $selectQuery = $selectQueryWrapper->subSelect();
        $selectQuery
            ->select(
                $selectQuery->alias($selectQuery->expr->count($this->dbHandler->quoteColumn('id', 'ezcontentclass')), 'cnt')
            )
            ->from(
                $this->dbHandler->quoteTable('ezcontentclass')
            )
            ->where(
                $selectQuery->expr->like(
                    $this->dbHandler->quoteColumn('identifier', 'ezcontentclass'),
                    $selectQuery->bindValue($baseIdentifier . '%')
                )
            );

        // wrap counting query to be able to use it in update (MySQL workaround)
        $selectQueryWrapper
            ->select('t1.cnt')
            ->from(
                $selectQueryWrapper->alias($selectQuery, 't1')
            );

        // update identifier
        $updateQuery->update($this->dbHandler->quoteTable('ezcontentclass'))
            ->set(
                $this->dbHandler->quoteColumn('identifier'),
                $updateQuery->expr->concat(
                    $updateQuery->bindValue($baseIdentifier),
                    $selectQueryWrapper
                )
            )
            ->where(
                $updateQuery->expr->eq(
                    $this->dbHandler->quoteColumn('id', 'ezcontentclass'),
                    $updateQuery->bindValue($typeId)
                )
            );
        $statement = $updateQuery->prepare();
        $statement->execute();

        // retrieve new generated identifier
        $query = $this->dbHandler->createSelectQuery();
        $query
            ->select($this->dbHandler->quoteColumn('identifier'))
            ->from($this->dbHandler->quoteTable('ezcontentclass'))
            ->where(
                $query->expr->eq(
                    $this->dbHandler->quoteColumn('id', 'ezcontentclass'),
                    $query->bindValue($typeId)
                )
            );
