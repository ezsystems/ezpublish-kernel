...
class Integer implements Converter
{
    ...
    public function toFieldDefinition( StorageFieldDefinition $storageDef, FieldDefinition $fieldDef )
    {
        ...
        {
            if ( ( !empty( $storageDef->dataInt1 ) || ($storageDef->dataInt4 === self::HAS_MIN_VALUE || $storageDef->dataInt4 === self::HAS_BOTH_VALUE)) )
            {
                $fieldDef->fieldTypeConstraints->validators[self::FLOAT_VALIDATOR_IDENTIFIER]['minIntegerValue'] = $storageDef->dataInt1;
            }

            if ( ( !empty( $storageDef->dataInt2 ) || ($storageDef->dataInt4 === self::HAS_MAX_VALUE || $storageDef->dataInt4 === self::HAS_BOTH_VALUE)) )
            {
