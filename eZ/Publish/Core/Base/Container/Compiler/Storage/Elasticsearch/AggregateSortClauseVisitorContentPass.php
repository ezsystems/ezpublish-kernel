...
class AggregateSortClauseVisitorContentPass implements CompilerPassInterface
{
    ...
    {
        if ( !$container->hasDefinition( 'ezpublish.persistence.elasticsearch.search.content.sort_clause_visitor.aggregate' ) )
        {
            return;
        }
        ...
        foreach ( $container->findTaggedServiceIds( 'ezpublish.persistence.elasticsearch.search.content.sort_clause_visitor' ) as $id => $attributes )
        {
