    ...
    public function configure(View $view)
    {
        foreach ($this->providerRegistry->getViewProviders($view) as $viewProvider) {
            if ($providerView = $viewProvider->getView($view)) {
                ...
                if ($providerView->getTemplateIdentifier() !== null) {
                    $view->setTemplateIdentifier($providerView->getTemplateIdentifier());
                }

                if ($providerView->getControllerReference() !== null) {
                    $view->setControllerReference($providerView->getControllerReference());
                }
