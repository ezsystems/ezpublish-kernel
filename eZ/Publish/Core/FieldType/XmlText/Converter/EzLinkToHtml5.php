...
class EzLinkToHtml5 implements Converter
{
    ...
    public function convert( DOMDocument $xmlDoc )
    {
        ...
        foreach ( $elements as $element )
        {
            ...
            if ( !$element->hasAttribute( 'url' ) && $element->hasAttribute( $this->tmpPrefix . 'url' ) )
            {
                $element->setAttribute( 'url', $element->getAttribute( $this->tmpPrefix . 'url' ) );
            }
    ...
    protected function elementHasAttribute( DomElement $element, $attributeName )
    {
        ...
        if ( $element->hasAttribute( $this->tmpPrefix . $attributeName ) )
        {
            return true;
        }
    ...
    protected function getElementAttribute( DomElement $element, $attributeName )
    {
        ...
        // Second return for embed
        return $element->getAttribute( $this->tmpPrefix . $attributeName );
    }
}
