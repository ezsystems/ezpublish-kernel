...
class Expanding implements Converter
{
    ...
    {
        $fragment = $this->expandElement( $document, $paragraph );
    ...
    protected function expandElement( DOMDocument $document, DOMElement $element )
    {
        ...
        foreach ( $element->childNodes as $node )
        {
            ...
            if ( $this->untangleNode( $fragment, $element, $expandingElement, $node ) )
            {
                continue;
            }
            ...
            if ( $node->nodeType === XML_ELEMENT_NODE )
            {
                ...
                foreach ( $subFragment->childNodes as $subNode )
                {
                    ...
                    if ( !$this->untangleNode( $fragment, $element, $expandingElement, $subNode ) )
                    {
    ...
    )
    {
        ...
        if ( $this->isEntangled( $node ) )
        {
            ...
            if ( $this->isEntangler( $element, $node ) )
            {
            ...
            {
                $expandingElement->appendChild( $node->cloneNode( true ) );
                $expandingElement->setAttribute( "ez-inherit-entanglement", 1 );
                $fragment->appendChild( $expandingElement );
     ...
     * @return boolean
     */
    protected function isEntangled( DOMNode $node )
    {
        return (
            isset( $this->containmentMap[$node->localName] )
            || ( $node instanceof DOMElement && $node->hasAttribute( "ez-inherit-entanglement" ) )
        );
    }
     ...
     * @return boolean
     */
    protected function isEntangler( DOMElement $element, DOMNode $node )
    {
        return (
