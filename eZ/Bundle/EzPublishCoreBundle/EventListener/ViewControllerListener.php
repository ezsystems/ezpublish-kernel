...
class ViewControllerListener implements EventSubscriberInterface
{
    ...
    public function getController(FilterControllerEvent $event)
    {
        ...
        if (($viewBuilder = $this->viewBuilderRegistry->get($request->attributes->get('_controller'))) === null) {
            return;
        }

        $parameterEvent = new ViewBuilderParameterCollectionEvent(clone $request);
        $this->eventDispatcher->dispatch('view.builder_parameter_collection', $parameterEvent);
