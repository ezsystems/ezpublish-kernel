<?php
/**
 * File containing the SetupWizard class.
 *
...
namespace eZ\Bundle\EzPublishLegacyBundle\Features\Context\SubContexts;
 ...
 * Class SetupWizard
 *
 * This class is the Context for setup wizard steps
 */
class SetupWizard extends BehatContext
{
    /**
    ...
    public function iSelectPackage( $packageName, $version )
    {
        ...
        $field = $this->getMainContext()->getSession()->getPage()->findField( $package );
        Assertion::assertNotNull( $field, "Couldn't find '$package' field" );
        $this->getMainContext()->browserFillField( $field );
    ...
    public function iSeeImported( $packageName, $version )
    {
        ...
        $el = $this->getMainContext()->getSession()->getPage()->find(
            "xpath",
            $packageXpath
        );
        ...
        $importElement = $this->getMainContext()->findElmentAfterElement(
            $this->getMainContext()->findRow( $el )->findAll( "xpath", "td" ),
            $packageXpath,
            "//*[text() = 'Imported']"
        );
    ...
    {
        $packages = $this->getMainContext()->convertTableToArrayOfData( $packagesTable );
